...
public class AdapterKeyValueCommerceInformation extends ArrayAdapter<ModelKeyValueInformation> implements View.OnClickListener{
        ...
        try {
            ...
            switch (v.getId()) {
                case R.id.img_assetedit:
                    ...
                    new DialogAddKeyValueCommerceItem(context, item, fragmentCommerce, jsonKey, key, value).show();
                    break;
            ...
            builder.setPositiveButton(
                    ...
                    new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int id) {
                            try {
                                dialog.cancel();
                                JSONArray oldlist = fragmentCommerce.farmercommerceData.getJSONArray(jsonKey);
                                JSONArray newlist = new JSONArray();
                                for (int i = 0; i < oldlist.length(); i++) {
                                    if (Id != oldlist.getJSONObject(i).getInt("Id"))
                                        newlist.put(oldlist.get(i));
                                }
                                fragmentCommerce.farmercommerceData.put(jsonKey, newlist);
