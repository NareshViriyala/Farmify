	...
	SELECT @agent_id = CASE WHEN [key] = 'agent_id' THEN [value] ELSE @agent_id END
	 ...
	 BEGIN
		...
		BEGIN
			...
			SELECT @farmer_id
				 , JSON_QUERY(@partner_data, '$.PartnerData')
				 , GETDATE()
				 , GETDATE()
				 , @agent_id
		...
		BEGIN --Aadhar already exists, so return the farmer information
			...
			SELECT @output = @farmer_id, @desc = '{"agent_id":'+CAST(@agent_id AS NVARCHAR)+',"individual_data":'+ISNULL(@individual_data,'')+',"bank_data":'+ISNULL(@bank_data,'')+',"social_data":'+ISNULL(@social_data,'')+',"commerce_data":'+ISNULL(@commerce_data,'')+',"partner_data":'+ISNULL(@partner_data,'')+'}'
			RETURN
		END
	 END
	 ...
	 BEGIN 
		...
		UPDATE dbo.tbl_farmer_social
		   SET FacebookID = JSON_VALUE(@social_data, '$.FacebookID')
		...
		UPDATE dbo.tbl_farmer_partner
		   SET PartnerData = JSON_QUERY(@partner_data, '$.PartnerData')
		     , LastModified = GETDATE()
			 , LastModifiedBy = @agent_id
		 WHERE farmer_id = @farmer_id
